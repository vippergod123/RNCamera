...
class Camera1 extends CameraViewImpl implements MediaRecorder.OnInfoListener,
   ...
   private long currentTimeCapture = -1;
   ...
   boolean setAspectRatio(final AspectRatio ratio) {
      if (mAspectRatio == null || !isCameraOpened()) {
         ...
         if (sizes == null) {
            ...
            Log.w("CAMERA_1::",
                  "setAspectRatio received an unsupported value and will be ignored.");
         } else {
            mAspectRatio = ratio;
            mBgHandler.post(new Runnable() {
               @Override
   ...
   @Override
   void stopLiveness(final Context context) {
      if (context == null) return;
   ...
   void takePictureInternal(final ReadableMap options) {
      ...
      if (!mIsRecording.get() && isPictureCaptureInProgress.compareAndSet(false, true)) {
         ...
         try {
            if (options.hasKey("orientation") && options.getInt(
                  "orientation") != Constants.ORIENTATION_AUTO) {
            ...
            mCamera.takePicture(null, null, null, new Camera.PictureCallback() {
               ...
               public void onPictureTaken(byte[] data, Camera camera) {
                  ...
                  synchronized (Camera1.this) {
                     if (mCamera != null) {
                        if (options.hasKey("pauseAfterCapture") && !options.getBoolean(
                              "pauseAfterCapture")) {
                           mCamera.startPreview();
                           mIsPreviewActive = true;
                           if (mIsScanning) {
   ...
   boolean record(String path, int maxDuration, int maxFileSize, boolean recordAudio,
                  CamcorderProfile profile, int orientation, int fps) {
      ...
      if (!isPictureCaptureInProgress.get() && mIsRecording.compareAndSet(false, true)) {
         ...
         try {
            ...
            int deviceOrientation = displayOrientationToOrientationEnum(mDeviceOrientation);
            mCallback.onRecordingStart(path,
                                       mOrientation != Constants.ORIENTATION_AUTO ? mOrientation : deviceOrientation,
                                       deviceOrientation);
   ...
   void adjustCameraParameters() {
      ...
      if (sizes == null) { // Not supported
         Log.w("CAMERA_1::",
               "adjustCameraParameters received an unsupported aspect ratio value and will be ignored.");
   ...
   private void setUpMediaRecorder(String path, int maxDuration, int maxFileSize,
      ...
      mMediaRecorder.setOrientationHint(calcCameraRotation(
            mOrientation != Constants.ORIENTATION_AUTO ? orientationEnumToRotation(
                  mOrientation) : mDeviceOrientation));
   ...
   private boolean isCompatibleWithDevice(int fps) {
      ...
      Log.w("CAMERA_1::",
            "fps (framePerSecond) received an unsupported value and will be ignored.");
      return false;
   }
