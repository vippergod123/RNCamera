...
public class CameraModule extends ReactContextBaseJavaModule {
      ...
      return Collections.unmodifiableMap(new HashMap<String, Object>() {
         {
            ...
            put("GoogleVisionBarcodeDetection",
                Collections.unmodifiableMap(new HashMap<String, Object>() {
                   {
   ...
   public void takePicture(final ReadableMap options, final int viewTag, final Promise promise) {
      ...
      uiManager.addUIBlock(new UIBlock() {
         @Override
         public void execute(NativeViewHierarchyManager nativeViewHierarchyManager) {
            RNCameraView cameraView = (RNCameraView) nativeViewHierarchyManager.resolveView(
                  viewTag);
            try {
               if (cameraView.isCameraOpened()) {
   ...
   @ReactMethod
   public void startLiveness(final ReadableMap options, final int viewTag, final Promise promise) {
      final ReactApplicationContext context = getReactApplicationContext();
      final File cacheDirectory = mScopedContext.getCacheDirectory();
      UIManagerModule uiManager = context.getNativeModule(UIManagerModule.class);
      ...
      uiManager.addUIBlock(new UIBlock() {
         ...
         public void execute(NativeViewHierarchyManager nativeViewHierarchyManager) {
            ...
            try {
               cameraView = (RNCameraView) nativeViewHierarchyManager.resolveView(viewTag);
               if (cameraView.isCameraOpened()) {
                  cameraView.startLiveness();
                  promise.resolve("startLiveness in execute CameraModule.java!!" + viewTag);
               } else {
                  promise.reject("E_CAMERA_UNAVAILABLE", "Camera is not running");
               }
            } catch (Exception e) {
   ...
   @ReactMethod
   public void stopLiveness(final int viewTag, final Promise promise) {
      final ReactApplicationContext context = getReactApplicationContext();
      final File cacheDirectory = mScopedContext.getCacheDirectory();
      UIManagerModule uiManager = context.getNativeModule(UIManagerModule.class);
      ...
      uiManager.addUIBlock(new UIBlock() {
         ...
         public void execute(NativeViewHierarchyManager nativeViewHierarchyManager) {
            ...
            try {
               cameraView = (RNCameraView) nativeViewHierarchyManager.resolveView(viewTag);
               if (cameraView.isCameraOpened()) {
                  cameraView.stopLiveness();
                  promise.resolve("stopLiveness in execute CameraModule.java!!" + viewTag);
               } else {
                  promise.reject("E_CAMERA_UNAVAILABLE", "Camera is not running");
               }
            } catch (Exception e) {
   ...
   @ReactMethod
   public void getAvailablePictureSizes(final String ratio, final int viewTag,
      ...
      uiManager.addUIBlock(new UIBlock() {
         ...
         public void execute(NativeViewHierarchyManager nativeViewHierarchyManager) {
            ...
            try {
               ...
               WritableArray result = Arguments.createArray();
               if (cameraView.isCameraOpened()) {
                  SortedSet<Size> sizes = cameraView.getAvailablePictureSizes(
                        AspectRatio.parse(ratio));
                  for (Size size : sizes) {
               ...
               promise.reject("E_CAMERA_BAD_VIEWTAG",
                              "getAvailablePictureSizesAsync: Expected a Camera component");
            }
         }
      });
   }
   ...
   public void checkIfRecordAudioPermissionsAreDefined(final Promise promise) {
      try {
         PackageInfo info = getCurrentActivity().getPackageManager().getPackageInfo(
               getReactApplicationContext().getPackageName(), PackageManager.GET_PERMISSIONS);
